...
@component('common-components.breadcrumb')
    ...
    @slot('title')   تعديل بيانات  المقايسه     @endslot
@endcomponent
  ...
  <div class="row">
        ...
        @endif
                ...
                <div id="addproduct-accordion" class="custom-accordion">
                        <div class="card">
                            <form id="form" method="post" action="{{url('/admin/edit-mokaysa/'.$mokaysa_detals->id)}}" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                                ...
                                                <div class="form-group row">
                                                    <div class="col-md-12">
                                                        <label  for="example-month-input" class="col-form-label">اسم  المقايسه  </label>
                                                            <input  class=" form-control" name="name" placeholder="أسم المقايسه " value="{{$mokaysa_detals->name}}"/>
                                                             <input  type="hidden" name="project_id" value="{{$projectDetails->id}}" />
                                                             <input  type="hidden" name="mokaysa_id" value="{{$mokaysa_detals->id}}" />
                                                             <input type="hidden" name="status" value="{{$mokaysa_detals->status}}" />
                                                        </div>
                                                </div>
                                                            ...
                                                            <div data-repeater-item class="outer">
                                                                ...
                                                                <div class="inner-repeater mb-12">
                                                                    <div data-repeater-list="inner-group" class="inner form-group">
                                                                        ...
                                                                        @php
                                                                            $owners = \App\MokaysaOwnerModel::where('mokaysa_id',$mokaysa_detals->id)->get();
                                                                        @endphp
                                                                        @if(!empty($owners))
                                                                            @foreach($owners as $owner)
                                                                                <div data-repeater-item class="inner mb-3 row">
                                                                                    <input type="text" class="inner form-control  inner-in  col-md-4" name="owners" value="{{$owner->owner_name}}" />
                                                                                    <input data-repeater-delete type="button" class="btn btn-primary btn-block inner  col-md-3" value="حذف"/>
                                                                                </div>
                                                                            @endforeach
                                                                        @else
                                                                            <input type="text" class="inner form-control  inner-in" name="owners" placeholder="أضافه أسم {{($mokaysa_detals->status == 0 ) ?   'مالك ' : 'باطن' }}"/>

                                                                        @endif
                                                                    </div>

                                                                    <input data-repeater-create type="button" class="btn btn-success inner" value="أضافه {{($mokaysa_detals->status == 0 ) ?   'مالك ' : 'باطن' }}  جديد "/>
                                                                </div>
